...
extern "C" {
    ...
    void _MatrixOS_Color_Color___init__(PikaObj *self, PikaTuple* val) {
        ...
        if(args_size == 1)
        {
            ...
            createCppValueInPikaObj<Color>(self, (uint32_t)hex);
        }
        else if(args_size == 3)
        {
            // RGB constructor
            ...
            createCppValueInPikaObj<Color>(self, (uint8_t)r, (uint8_t)g, (uint8_t)b);
        }
        else if(args_size == 4)
        {
            // RGBW constructor
            ...
            createCppValueInPikaObj<Color>(self, (uint8_t)r, (uint8_t)g, (uint8_t)b, (uint8_t)w);
        }
        else
        {
            // Black
            createCppValueInPikaObj<Color>(self, 0);
        }
    }
    ...
    PikaObj* _MatrixOS_Color_Color_FromRGBW(PikaObj *self, int r, int g, int b, int w) {
        ...
        createCppValueInPikaObj<Color>(new_color, (uint8_t)r, (uint8_t)g, (uint8_t)b, (uint8_t)w);
        return new_color;
    }
    ...
    PikaObj* _MatrixOS_Color_Color_FromHex(PikaObj *self, int wrgb) {
        PikaObj* new_color = newNormalObj(New__MatrixOS_Color_Color);
        createCppValueInPikaObj<Color>(new_color, (uint32_t)wrgb);
        return new_color;
    }

    // Color getter methods
    int _MatrixOS_Color_Color_R(PikaObj *self) {
        Color* color = getCppValuePtrInPikaObj<Color>(self);
        if (!color) return 0;
        return color->R;
    }
    ...
    PikaObj* _MatrixOS_Color_Color_Dim(PikaObj *self, pika_float factor) {
        ...
        PikaObj* new_color = newNormalObj(New__MatrixOS_Color_Color);
        copyCppValueIntoPikaObj<Color>(new_color, dimmed);
        return new_color;
    }
    ...
    PikaObj* _MatrixOS_Color_Color_Scale(PikaObj *self, pika_float factor) {
        ...
        PikaObj* new_color = newNormalObj(New__MatrixOS_Color_Color);
        copyCppValueIntoPikaObj<Color>(new_color, scaled);
        return new_color;
    }

    // Color operators
    pika_bool _MatrixOS_Color_Color___eq__(PikaObj *self, PikaObj* other) {
        Color* color1 = getCppValuePtrInPikaObj<Color>(self);
        Color* color2 = getCppValuePtrInPikaObj<Color>(other);
