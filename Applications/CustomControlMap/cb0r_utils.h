...
inline bool cb0r_get_check_type(cb0r_t cbor, uint32_t index, cb0r_s* cbor_data, cb0r_e type) {
  ...
  {
    MLOGE("cb0r_get_check_type", "Incorrect action data type - Expected: %s, Got: %s", cb0r_type_to_string(type).c_str(),
          cb0r_type_to_string(cbor_data->type).c_str());
    return false;
  }
  return true;
}
...
inline bool cb0r_next(cb0r_t array, cb0r_t prev, cb0r_s* cbor_data) {
  ...
  {
    cb0r(array->start + array->header, array->end, 0, cbor_data); // You can pass in the same array as prev to get the first element. Useful
                                                                  // if you don't have a previous element yet in a for loop.
  }
  else
  {
...
inline bool cb0r_next_check_type(cb0r_t array, cb0r_t prev, cb0r_s* cbor_data, cb0r_e type) {
  ...
  if (cbor_data->type != type)
  {
    MLOGE("cb0r_next_check_type", "Incorrect action data type - Expected: %s, Got: %s", cb0r_type_to_string(type).c_str(),
          cb0r_type_to_string(cbor_data->type).c_str());
    return false;
  }
