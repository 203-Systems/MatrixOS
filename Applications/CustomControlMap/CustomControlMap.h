...
private:
  ...
  void SendError(uint8_t command, uint8_t errorCode);
  void SendAck(uint8_t command, uint8_t ackCode = 0);
...
enum HIDCommand {           // MSB is 1 for Write/Response, 0 for Read/Request
  ...
  UAD_STATUS =
      0x02,         // Gets the active UAD info like loaded and size. Sets the UAD info for the new UAD payload (will clear the current UAD)
