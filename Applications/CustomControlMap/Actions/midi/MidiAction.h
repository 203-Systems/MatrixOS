...
enum class AnalogSource : uint8_t { Momentary = 0, Persistent = 1, Toggle = 2, KeyForce = 3, Invalid = 0xFF };
...
static bool LoadData(cb0r_t actionData, MidiAction* action) {
  ...
  if (action->type == MidiType::Start || action->type == MidiType::Continue || action->type == MidiType::Stop ||
      action->type == MidiType::Reset)
  {
    action->source = AnalogSource::Persistent;
    return true;
  }
  ...
  // Get Channel
  if (action->type == MidiType::Note || action->type == MidiType::ControlChange || action->type == MidiType::ProgramChange ||
      action->type == MidiType::ChannelPressure || action->type == MidiType::PitchBend || action->type == MidiType::RPN ||
      action->type == MidiType::NRPN)
  {
  ...
  // Get Note or Control
  if (action->type == MidiType::Note || action->type == MidiType::ControlChange || action->type == MidiType::ProgramChange ||
      action->type == MidiType::RPN || action->type == MidiType::NRPN)
  {
    if (!cb0r_next_check_type(actionData, &cbor_data, &cbor_data, CB0R_INT))
    {
  ...
  if (action->type == MidiType::Note || action->type == MidiType::ControlChange || action->type == MidiType::ChannelPressure ||
      action->type == MidiType::PitchBend || action->type == MidiType::RPN || action->type == MidiType::NRPN)
  {
    if (!cb0r_next_check_type(actionData, &cbor_data, &cbor_data, CB0R_INT))
    {
...
static bool KeyEvent(UADRuntime* uadRT, ActionInfo* actionInfo, cb0r_t actionData, KeyInfo* keyInfo) {
  ...
  case AnalogSource::Toggle: {
    ...
    if (keyInfo->State() == PRESSED)
    {
      ...
      groupRegister = (groupRegister & 0xFFFFFFF0) +
                      (registerValue & 1); // We use the lower 4 bits for the group register as the LED index for the action driven LED
  ...
  {
  case MidiType::Note: {
    MatrixOS::MIDI::Send(MidiPacket::NoteOn(data.channel, data.note, outputValue));
    return true;
  }
  case MidiType::ControlChange: {
    MatrixOS::MIDI::Send(MidiPacket::ControlChange(data.channel, data.control, outputValue));
    return true;
  }
  ...
  case MidiType::ChannelPressure: {
    MatrixOS::MIDI::Send(MidiPacket::ChannelPressure(data.channel, outputValue));
    return true;
  }
  case MidiType::PitchBend: {
    MLOGD(TAG, "Pitch Bend: %d", outputValue);
    MatrixOS::MIDI::Send(MidiPacket::PitchBend(data.channel, outputValue));
    return true;
  }
  case MidiType::SysEx: {
    vector<uint8_t> processedSysex;
    processedSysex.resize(data.sysex_length);
    ...
    for (size_t i = 0; i < data.sysex_length; i++)
    {
      ...
      {
        processedSysex[i] = 0x00; // Input mapping, not implemented
      }
      else
      {
        processedSysex[i] = data.sysex_data[i];
      }
    }

    // TODO: Fix this for other ports!
    MatrixOS::MIDI::SendSysEx(EMidiPortID::MIDI_PORT_USB, data.sysex_length, processedSysex.data(), false);
  ...
  case MidiType::RPN: {
    ...
    MatrixOS::MIDI::Send(MidiPacket::ControlChange(data.channel, 6, (outputValue >> 7) & 0x7F));
    MatrixOS::MIDI::Send(MidiPacket::ControlChange(data.channel, 38, outputValue & 0x7F));
    return true;
  }
  case MidiType::NRPN: {
