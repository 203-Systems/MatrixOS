...
void BrightnessControl::Start() {
  ...
  brightnessSelector.SetIndividualColorFunc([&](uint16_t index) -> Color {
    return (map[index] > threshold ? Color(0xFF0000) : Color::White).DimIfNot(map[index] <= MatrixOS::UserVar::brightness.value);
  });
  ...
  brightnessDisplay.SetColorFunc([&](uint16_t digit) -> Color {
    return digit % 2 ? Color::White : (MatrixOS::UserVar::brightness.value > threshold) ? Color(0xFF0000) : GLOBAL_BRIGHTNESS_COLOR;
  });
  ...
  vector<float> ledBrightnessMultiplier = {0.0, 0.1, 0.25, 0.5, 0.75, 1.0, 1.2, 1.5,
                                           2.0, 2.5, 3.0,  4.0, 5.0,  6.0, 8.0, std::numeric_limits<float>::infinity()};
  ...
  multiplierSelector.OnChange(
      [&](float value) -> void { MatrixOS::LED::SetBrightnessMultiplier(Device::LED::partitions[ledPartitionSelected].name, value); });
  ...
  ledPartitionSelector.SetIndividualColorFunc(
      [&](uint16_t index) -> Color { return index == 0 ? GLOBAL_BRIGHTNESS_COLOR : PARTITION_BRIGHTNESS_COLOR; });
  ledPartitionSelector.SetIndividualNameFunc(
      [&](uint16_t index) -> string { return index == 0 ? "Global" : Device::LED::partitions[index].name; });
