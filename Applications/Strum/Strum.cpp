...
void Strum::Setup(const vector<string>& args) {
  ...
  StrumChordVisualizer scaleVisualizer(&strumBarConfigs[activeLayout].root, (uint16_t*)&strumBarConfigs[activeLayout].chord,
                                       Color(0x8000FF), Color(0xFF00FF));
  ...
  major7thBtn.SetColorFunc(
      [&]() -> Color { return Color(0xFFBF00).DimIfNot(strumBarConfigs[activeLayout].chord == EChord::MAJOR_SEVENTH); });
  ...
  minor7thBtn.SetColorFunc(
      [&]() -> Color { return Color(0xFFFF00).DimIfNot(strumBarConfigs[activeLayout].chord == EChord::MINOR_SEVENTH); });
  ...
  diminishedBtn.SetColorFunc(
      [&]() -> Color { return Color(0x00AAFF).DimIfNot(strumBarConfigs[activeLayout].chord == EChord::DIMINISHED); });
