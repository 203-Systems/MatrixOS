...
bool SerializeSequenceMeta(const SequenceMeta& meta, File& file) {
  ...
  cb_write_text(buffer, "color");
  cb_write_uint(buffer, CB0R_INT, meta.color.RGB());
  cb_write_text(buffer, "clock");
  cb_write_bool(buffer, meta.clockOutput);
  MLOGD("SequenceMeta", "Serialize header color=0x%06X clock=%d tracks=%u", meta.color.RGB(), meta.clockOutput ? 1 : 0,
        (unsigned)meta.tracks.size());
  ...
  for (size_t i = 0; i < meta.tracks.size(); ++i)
  {
    ...
    if (t.mode == SequenceTrackMode::NoteTrack || t.mode == SequenceTrackMode::DrumTrack)
    {
      ...
      cb_write_text(buffer, "type");
      cb_write_uint(buffer, CB0R_INT, static_cast<uint8_t>(n.type));
      cb_write_text(buffer, "cscale");
      cb_write_bool(buffer, n.customScale);
      cb_write_text(buffer, "enforce");
      cb_write_bool(buffer, n.enforceScale);
      cb_write_text(buffer, "scale");
      cb_write_uint(buffer, CB0R_INT, n.scale);
      cb_write_text(buffer, "root");
      cb_write_uint(buffer, CB0R_INT, n.root);
      cb_write_text(buffer, "offset");
      cb_write_uint(buffer, CB0R_INT, n.rootOffset);
      cb_write_text(buffer, "oct");
      cb_write_uint(buffer, CB0R_INT, n.octave);
      MLOGD("SequenceMeta", "Serialize track %u mode=note color=0x%06X vel=%d type=%u scale=%u root=%u off=%u oct=%u", (unsigned)i,
            t.color.RGB(), t.velocitySensitive ? 1 : 0, (unsigned)n.type, (unsigned)n.scale, (unsigned)n.root, (unsigned)n.rootOffset,
            (unsigned)n.octave);
    }
    else // CC track
    {
      ...
      cb_write_text(buffer, "param");
      cb_write_uint(buffer, CB0R_INT, c.parameter);
...
static bool ParseTrackMeta(cb0r_s m, SequenceMetaTrack& t) {
  cb0r_s item;
  // ["t", color, vel, mode, cfg]
  if (m.type != CB0R_ARRAY || m.length < 5)
    return false;
  if (!cb0r_get(&m, 1, &item) || item.type != CB0R_INT)
    return false;
  t.color = Color(item.value);
  if (!cb0r_get(&m, 2, &item))
    return false;
...
bool DeserializeSequenceMeta(File& file, SequenceMeta& out) {
  ...
  {
    MLOGE("SequenceMeta", "Header parse failed");
    return false;
  }
  size_t consumed = (size_t)(root.end - (buffer.data() + offset));
  if (root.type != CB0R_MAP || consumed == 0 || offset + consumed > buffer.size())
  {
    MLOGE("SequenceMeta", "Header invalid");
    return false;
  }
  cb0r_s item;
  if (cb0r_find(&root, CB0R_UTF8, 5, (uint8_t*)"color", &item))
    out.color = Color(item.value);
  if (cb0r_find(&root, CB0R_UTF8, 5, (uint8_t*)"clock", &item))
    out.clockOutput = (item.type == CB0R_TRUE) || (item.type == CB0R_INT && item.value != 0);
  ...
  while (offset < buffer.size())
  {
    ...
    if (consumed == 0 || offset + consumed > buffer.size())
    {
      MLOGE("SequenceMeta", "Track %u consumed invalid", trackId);
      return false;
    }
    ...
    {
      MLOGE("SequenceMeta", "Tag missing/invalid (track %u)", trackId);
      return false;
    }
    string tag((char*)(tagNode.start + tagNode.header), tagNode.length);
    if (tag != "t")
    {
      MLOGE("SequenceMeta", "Unknown tag %s (trackId=%u)", tag.c_str(), trackId);
      return false;
    }

    SequenceMetaTrack mt{};
    if (!ParseTrackMeta(root, mt))
    {
      MLOGE("SequenceMeta", "ParseTrackMeta failed at %u", trackId);
      return false;
    }
    if (trackId >= out.tracks.size())
      out.tracks.resize(trackId + 1);
    ...
    if (mt.mode == SequenceTrackMode::NoteTrack || mt.mode == SequenceTrackMode::DrumTrack)
    {
      ...
      MLOGD("SequenceMeta", "Track %u note cfg type=%u cscale=%d enforce=%d scale=%u root=%u offset=%u oct=%u", trackId, (unsigned)n.type,
            n.customScale ? 1 : 0, n.enforceScale ? 1 : 0, (unsigned)n.scale, (unsigned)n.root, (unsigned)n.rootOffset, (unsigned)n.octave);
    }
    else
    {
...
bool GetColorFromSequenceMetaFile(File& file, Color& outColor) {
  ...
  cb0r_s root{};
  if (!cb0r_read(buffer.data(), buffer.size(), &root) || root.type != CB0R_MAP)
    return false;
