...
void Sequence::UpdateTiming() {
  ...
  MLOGD("Sequence",
        ...
        data.bpm, data.swing, stepDivision, data.patternLength, barLength, data.beatsPerBar, data.beatUnit, pulsesPerStep,
        (unsigned long)usPerPulse[0], (unsigned long)usPerPulse[1], (unsigned long)usPerClock);
}
...
void Sequence::PlayFrom(uint8_t track, uint8_t clip, uint8_t pattern, uint8_t step) {
  ...
  if (pattern >= GetPatternCount(track, clip))
    return;
...
bool Sequence::PatternOffsetNotesInRange(SequencePattern* pattern, uint16_t startTime, uint16_t endTime, int8_t offset) {
  ...
  if (eventsToModify.empty() && eventsToRemove.empty())
    return false;
...
bool Sequence::PatternQuantize(SequencePattern* pattern, SequencePattern* patternNext, uint16_t stepPulse) {
  ...
  auto quantizeVal = [stepPulse](uint16_t val) -> uint16_t { return (val + stepPulse / 2) / stepPulse * stepPulse; };
...
bool Sequence::PatternNudge(SequencePattern* pattern, int16_t offsetPulse) {
  ...
  if (normalized < 0)
  {
    normalized += patternLengthPulses;
  }
...
bool Sequence::PatternNudgeInRange(SequencePattern* pattern, uint16_t startTime, uint16_t length, int16_t offsetPulse,
  ...
  if (eventsToMove.empty() && eventsToRemove.empty())
    return false;
...
void Sequence::CopyPattern(uint8_t sourceTrack, uint8_t sourceClip, uint8_t sourcePattern, uint8_t destTrack, uint8_t destClip,
  ...
  if (!ClipExists(sourceTrack, sourceClip) || !ClipExists(destTrack, destClip))
    return;

  auto& sourcePatterns = data.tracks[sourceTrack].clips[sourceClip].patterns;
  if (sourcePattern >= sourcePatterns.size())
    return;
  ...
  if (destPattern == 255)
  {
    ...
    if (destPatterns.size() >= SEQUENCE_MAX_PATTERN_COUNT)
    {
      return;
    }
  ...
  else
  {
    ...
    auto& destPatterns = data.tracks[destTrack].clips[destClip].patterns;
    if (destPattern >= destPatterns.size())
      return;
...
void Sequence::RecordEvent(MidiPacket packet, uint8_t track) {
  ...
  if (track != 0xFF && track >= trackPlayback.size())
    return;
  ...
  if (track == 0xFF)
  {
    ...
    {
      if (data.tracks[i].channel != packet.Channel())
        continue;
      if (!GetEnabled(i))
        continue;
      targets.push_back(i);
...
void Sequence::ProcessTrack(uint8_t track) {
    ...
    for (auto eventIt = range.first; eventIt != range.second; ++eventIt)
    {
      ...
      case SequenceEventType::NoteEvent: {
        ...
        if (mapIt != trackPlayback[track].noteOffMap.end())
        {
          ...
          for (auto qIt = trackPlayback[track].noteOffQueue.begin(); qIt != trackPlayback[track].noteOffQueue.end();)
          {
            if (qIt->second == note)
            {
  ...
  while (!trackPlayback[track].noteOffQueue.empty() && trackPlayback[track].noteOffQueue.begin()->first <= pulseSinceStart)
  {
