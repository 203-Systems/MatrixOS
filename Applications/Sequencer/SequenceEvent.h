...
using SequenceEventData = std::variant<SequenceEventNote, SequenceEventCC>;
...
struct SequenceEvent {
  ...
  SequenceEvent(SequenceEventType type, const SequenceEventData& eventData) : eventType(type), data(eventData), recordLayer(0) {}
  ...
  static SequenceEvent Note(const uint8_t note, const uint8_t velocity, const bool aftertouch,
                            const uint16_t length = UINT16_MAX /*UINT16_MAX = auto-set to default step length*/);
