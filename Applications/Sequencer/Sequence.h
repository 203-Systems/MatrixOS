...
public:
  ...
  uint8_t GetBeatsPerBar() const {
    return data.beatsPerBar;
  }
  uint8_t GetBeatUnit() const {
    return data.beatUnit;
  }
  ...
  void CopyPattern(uint8_t sourceTrack, uint8_t sourceClip, uint8_t sourcePattern, uint8_t destTrack, uint8_t destClip,
                   uint8_t destPattern = 255); // if destPattern is 255, then will create new pattern
  ...
  bool PatternHasEventInRange(SequencePattern* pattern, uint16_t startTime, uint16_t endTime,
                              SequenceEventType type = SequenceEventType::Invalid);
  ...
  bool DualPatternQuantize(SequencePattern* pattern1, SequencePattern* pattern2, SequencePattern* patternNext,
                           uint16_t stepPulse); // patternNext to handle overflow
  ...
  bool PatternNudgeInRange(SequencePattern* pattern, uint16_t startTime, uint16_t length, int16_t offsetPulse, SequencePattern* prevPattern,
                           SequencePattern* nextPattern);
  ...
  uint16_t GetPulsesPerStep() const {
