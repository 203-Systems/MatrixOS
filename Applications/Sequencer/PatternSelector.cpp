...
Dimension PatternSelector::GetSize() {
...
bool PatternSelector::KeyEvent(Point xy, KeypadInfo* keypadInfo) {
  if (xy.y < 2) // Selector part
  {
    ...
    if (keypadInfo->state == KeypadState::Pressed)
    {
      ...
      bool copyActive = sequencer->CopyActive() && (sequencer->copySource.IsType(SequenceSelectionType::PATTERN) ||
                                                    sequencer->copySource.IsType(SequenceSelectionType::NONE));
...
bool PatternSelector::Render(Point origin) {
  ...
  for (uint8_t i = 0; i < 16; i++)
  {
    ...
    else if (i < patternCount)
    {
      ...
      bool isPatternCopySource = patternSelectedForCopy && sequencer->copySource.track == track && sequencer->copySource.clip == clip &&
