...
bool SequencerNotePad::IsEnabled() {
  bool enabled = (sequencer->currentView == Sequencer::ViewMode::Sequencer) &&
                 (sequencer->meta.tracks[sequencer->track].mode == SequenceTrackMode::NoteTrack ||
                  sequencer->meta.tracks[sequencer->track].mode == SequenceTrackMode::DrumTrack);
...
void SequencerNotePad::GenerateOctaveKeymap() {
  ...
  for (int8_t y = 0; y < dimension.y; y++)
  {
    ...
    {
      uint8_t id = uiY * dimension.x + x;
      if (nextNote > 127)
      { // If next note is out of range, fill with 255
        noteMap[id] = 255;
      }
      else if (!metaTrack.config.note.enforceScale)
      {                         // If enforce scale is false, just add the next note
      ...
      { // Find the next note that is in scale
        while (true)
        { // Find next key that we should put in
          NoteType inScale = InScale(nextNote);
          if (inScale == NoteType::ROOT_NOTE)
          {
            rootCount++;
...
void SequencerNotePad::GenerateDrumKeymap() {
  ...
  for (uint8_t i = 0; i < dimension.Area(); i++)
  {
    uint8_t x = i % dimension.x;
    uint8_t y = i / dimension.x;
    uint8_t uiY = dimension.y - y - 1;

    uint8_t note = 0;
    uint8_t offset = 36;
    if (TwoRowMode())
    {
      note = (x % 4) + uiY * 4 + offset;
      if (x >= 4)
      {
        note += 8;
      }
...
void SequencerNotePad::GenerateKeymap() {
  ...
  if (metaTrack.mode == SequenceTrackMode::NoteTrack)
  {
    c_aligned_scale_map = (((uint16_t)metaTrack.config.note.scale << metaTrack.config.note.root) +
                           (((uint16_t)metaTrack.config.note.scale & 0xFFF) >> (12 - metaTrack.config.note.root % 12))) &
                          0xFFF;
...
void SequencerNotePad::SequencerEvent(const MidiPacket& packet) {
  ...
  bool sequenceRunning =
      sequencer->sequence.Playing(track) || (sequencer->sequence.Playing() && sequencer->sequence.GetClocksTillStart() > 0);
  if (sequenceRunning && sequencer->sequence.RecordEnabled() && sequencer->sequence.ShouldRecord(track) && stepsSelected == 0)
  {
