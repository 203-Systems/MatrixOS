...
Dimension PatternPad::GetSize() {
  return Dimension(8, TwoPatternMode() ? 4 : 2);
}
...
bool PatternPad::Render(Point origin) {
  ...
  for (uint8_t i = 0; i < patternsToRender; i++)
  {
    ...
    // Render Step
    bool copyFilter =
        (sequencer->CopyActive() && sequencer->copySource.Selected() && !sequencer->copySource.IsType(SequenceSelectionType::STEP));
    ...
    // Render Copy Source
    if (sequencer->CopyActive() && sequencer->copySource.IsType(SequenceSelectionType::STEP) && sequencer->copySource.track == track &&
        sequencer->copySource.clip == clip && sequencer->copySource.pattern == patternIdx && sequencer->copySource.step < pattern->steps)
    {
