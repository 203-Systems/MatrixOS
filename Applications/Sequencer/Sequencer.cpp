...
void Sequencer::SequencerUI() {
  ...
  trackSelector.OnChange([&](uint8_t val) -> void { notePad.GenerateKeymap(); });
  trackSelector.SetEnableFunc(
      [&]() -> bool { return currentView != ViewMode::Session && currentView != ViewMode::Mixer && currentView != ViewMode::StepDetail; });
...
void Sequencer::SequencerMenu() {
  ...
  swingSelectorBtn.OnPress([&]() -> void { SwingSelector(); });
  ...
  timeSignatureSelector.OnPress([&]() -> void { TimeSignatureSelector(); });
  ...
  patternLengthSelectorBtn.OnPress([&]() -> void { PatternLengthSelector(); });
  ...
  sequenceBrowserBtn.SetColorFunc(
      [&]() -> Color { return MatrixOS::FileSystem::Available() ? ColorEffects::Rainbow(3000) : Color::Red.Dim(); });
  sequenceBrowserBtn.SetSize(Dimension(4, 1));
  sequenceBrowserBtn.OnPress(
      [&]() -> void { MatrixOS::FileSystem::Available() ? SequenceBrowser() : MatrixOS::UIUtility::TextScroll("No SD Card", Color::Red); });
...
void Sequencer::LayoutSelector() {
  ...
  scaleModeBtn.SetColorFunc([&]() -> Color {
    return noteColor.DimIfNot(meta.tracks[track].mode == SequenceTrackMode::NoteTrack &&
  ...
  drumModeBtn.SetColorFunc([&]() -> Color { return drumColor.DimIfNot(meta.tracks[track].mode == SequenceTrackMode::DrumTrack); });
  ...
  enforceScaleBtn.SetColorFunc([&]() -> Color { return Color(0xFFFFFF).DimIfNot(meta.tracks[track].config.note.enforceScale); });
  enforceScaleBtn.SetEnableFunc([&]() -> bool {
    return meta.tracks[track].mode == SequenceTrackMode::NoteTrack && (meta.tracks[track].config.note.type == SequenceNoteType::Scale ||
                                                                       meta.tracks[track].config.note.type == SequenceNoteType::Chromatic);
  });
  ...
  customScaleEnableBtn.SetColorFunc([&]() -> Color { return Color(0x00FFFF).DimIfNot(meta.tracks[track].config.note.customScale); });
  customScaleEnableBtn.SetEnableFunc([&]() -> bool { return meta.tracks[track].mode == SequenceTrackMode::NoteTrack && !noteMapTesting; });
  ...
  SequencerScaleVisualizer scaleVisualizer(Color(0xFF00FF), Color(0x8000FF), Color(0xFF0090));
  ...
  scaleVisualizer.SetEnableFunc([&]() -> bool { return meta.tracks[track].mode == SequenceTrackMode::NoteTrack && !noteMapTesting; });
  ...
  offsetModeBtn.SetEnableFunc([&]() -> bool { return meta.tracks[track].mode == SequenceTrackMode::NoteTrack && !noteMapTesting; });
  ...
  scaleSelectorBar.SetEnableFunc([&]() -> bool {
    return meta.tracks[track].mode == SequenceTrackMode::NoteTrack && !noteMapTesting && !meta.tracks[track].config.note.customScale;
  });
  ...
  chmTextDisplay.SetEnableFunc([&]() -> bool {
    return meta.tracks[track].mode == SequenceTrackMode::NoteTrack && meta.tracks[track].config.note.type == SequenceNoteType::Chromatic;
  });
  ...
  pioTextDisplay.SetEnableFunc([&]() -> bool {
    return meta.tracks[track].mode == SequenceTrackMode::NoteTrack && meta.tracks[track].config.note.type == SequenceNoteType::Piano;
  });
  ...
  drmTextDisplay.SetEnableFunc(
      [&]() -> bool { return (meta.tracks[track].mode == SequenceTrackMode::DrumTrack) && (noteMapTesting == false); });
  ...
  applyNoteBtn.SetColorFunc([&]() -> Color { return updateTextDisplay.IsEnabled() ? Color::White : updateColor; });
  applyNoteBtn.SetEnableFunc([&]() -> bool { return meta.tracks[track].mode == SequenceTrackMode::NoteTrack; });
  applyNoteBtn.OnPress([&]() -> void {
    ...
    for (size_t i = 0; i < meta.tracks.size(); ++i)
    {
      if (i == track)
        continue;
      if (meta.tracks[i].mode != SequenceTrackMode::NoteTrack)
        continue;
...
void Sequencer::TimeSignatureSelector() {
  ...
  beatsDecreaseBtn.SetColorFunc([&]() -> Color { return Color(0xFF00FF).DimIfNot(beatsPerBar > 1); });
  ...
  beatsIncreaseBtn.SetColorFunc([&]() -> Color { return Color(0x00FFFF).DimIfNot(beatsPerBar < 16); });
  ...
  beatUnitDecreaseBtn.SetColorFunc([&]() -> Color { return Color(0xFF00FF).DimIfNot(beatUnit > 1); });
  ...
  beatUnitIncreaseBtn.SetColorFunc([&]() -> Color { return Color(0x00FFFF).DimIfNot(beatUnit < 16); });
...
void Sequencer::PatternLengthSelector() {
  ...
  numDisplay.SetSpacing(1);
  numDisplay.SetEnableFunc([&]() -> bool { return !patternLengthTextDisplay.IsEnabled() && !updateTextDisplay.IsEnabled(); });
  ...
  patternLengthInput.SetColorFunc([&]() -> Color { return (MatrixOS::SYS::Millis() - updateTime < 1000) ? updateColor : color; });
  ...
  updatePatternsBtn.SetColorFunc([&]() -> Color { return updateTextDisplay.IsEnabled() ? Color::White : updateColor; });
...
void Sequencer::ShiftEventOccured() {
  if (shift[0])
  {
    shiftEventOccured[0] = true;
  }
  else if (shift[1])
  {
    shiftEventOccured[1] = true;
  }
}
...
bool Sequencer::Save(uint16_t slot) {
  ...
  if (slot >= SD_SLOT_MAX)
  {
    MLOGE("Sequencer", "Save - slot out of range %u", slot);
    return false;
  }
  if (!MatrixOS::FileSystem::Available())
  {
    MLOGE("Sequencer", "Save - filesystem not available");
    return false;
  }
  ...
  if (!metaOk)
  {
    MLOGE("Sequencer", "Save - write meta failed");
    return false;
  }
  ...
  if (!dataOk)
  {
    MLOGE("Sequencer", "Save - serialize stream failed");
    return false;
  }
...
bool Sequencer::Load(uint16_t slot) {
  ...
  if (slot >= SD_SLOT_MAX)
  {
    MLOGE("Sequencer", "Load - slot out of range %u", slot);
    return false;
  }
  if (!MatrixOS::FileSystem::Available())
  {
    MLOGE("Sequencer", "Load - filesystem not available");
    return false;
  }
  ...
  if (metaFile.Name().empty())
  {
    MLOGE("Sequencer", "Load - open meta fail %s", metaPath.c_str());
    return false;
  }
  ...
  if (!metaOk)
  {
    MLOGE("Sequencer", "Load - deserialize meta failed");
    return false;
  }
  ...
  if (!dataOk)
  {
    MLOGE("Sequencer", "Load - deserialize data failed");
    return false;
  }
...
bool Sequencer::Saved(uint16_t slot) {
  if (slot >= SD_SLOT_MAX)
  {
    MLOGD("Sequencer", "Saved slot out of range %u", slot);
    return false;
  }
  if (!MatrixOS::FileSystem::Available())
  {
...
void Sequencer::ConfirmSaveUI() {
  ...
  confirmSaveUI.SetPostRenderFunc([&]() -> void {
    ...
    if (saved || failed)
    {
      if (MatrixOS::SYS::Millis() - savedTime > 500)
      {
        confirmSaveUI.Exit();
      }
    }
...
void Sequencer::SequenceBrowser() {
  ...
  for (uint16_t slot = 0; slot < SD_SLOT_MAX && slot < 48; ++slot)
  {
    if (Saved(slot))
    {
      ...
      if (GetColorFromSequenceMetaFile(f, c))
        slotColors[slot] = c;
    }
  }
