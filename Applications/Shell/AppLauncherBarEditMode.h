...
public:
  ...
  void FolderSettings(uint8_t folderIdx) {
    ...
    colorPickerBtn.SetColorFunc([&]() -> Color { return shell->folder_colors[folderIdx]; });
    colorPickerBtn.OnPress([&]() -> void {
      Color newColor = shell->folder_colors[folderIdx];
      if (MatrixOS::UIUtility::ColorPicker(newColor))
      {
        shell->EnableFolder(folderIdx, newColor);
      }
    });
  ...
  virtual bool KeyEvent(Point xy, KeyInfo* keyInfo) {
    if (keyInfo->State() == RELEASED)
    {
      if (xy.x < 7)
      {
        uint8_t folderIdx = xy.x == 6 ? Shell::FOLDER_HIDDEN : xy.x;

        // Check if folder is active (exists)
        if (folderIdx != Shell::FOLDER_HIDDEN && shell->folder_colors[folderIdx] == Color(0x000000))
        {
          // Folder doesn't exist - enable it
          Color newColor = Color(0x000000);
          if (!MatrixOS::UIUtility::ColorPicker(newColor))
          {
        ...
        else if (shell->selected_app_id != 0)
        {
          ...
          shell->MoveAppToFolder(shell->selected_app_id, folderIdx);
          shell->selected_app_id = 0; // Clear selection
          MLOGD("AppLauncherBarEditMode", "Moved app to folder %d", folderIdx);
          // No app selected - just switch to the folder
        }
        else
        {
          shell->current_folder = folderIdx;
          MLOGD("AppLauncherBarEditMode", "Switched to folder %d", folderIdx);
        }
        return true;
      }
    }
