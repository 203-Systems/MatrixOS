...
{
void ExecuteAPP(uint32_t appId);
uint16_t GetApplicationCount();
} // namespace MatrixOS::SYS

void Shell::Setup(const vector<string>& args) {
#ifdef configUSE_FREERTOS_PROVIDED_HEAP
  MLOGD("Shell", "Matrix OS Free Heap Size: %.2fkb (%d%%)", xPortGetFreeHeapSize() / 1024.0f,
        xPortGetFreeHeapSize() * 100 / configTOTAL_HEAP_SIZE);
  MLOGD("Shell", "Matrix OS Minimum Free Heap Size: %.2fkb (%d%%)", xPortGetMinimumEverFreeHeapSize() / 1024.0f,
        xPortGetMinimumEverFreeHeapSize() * 100 / configTOTAL_HEAP_SIZE);
#endif
#ifdef ESP_PLATFORM
  multi_heap_info_t info;
  heap_caps_get_info(&info, MALLOC_CAP_DEFAULT);
  uint32_t totalHeapSize = heap_caps_get_total_size(MALLOC_CAP_DEFAULT);
  MLOGD("Shell", "Matrix OS Free Heap Size: %d (%.2fkb) (%d%%)", info.total_free_bytes, info.total_free_bytes / 1024.0f,
        info.total_free_bytes * 100 / totalHeapSize);
  MLOGD("Shell", "Matrix OS Lifetime Minimum Free Heap Size: %d (%.2fkb) (%d%%)", info.minimum_free_bytes,
        info.minimum_free_bytes / 1024.0f, info.minimum_free_bytes * 100 / totalHeapSize);
  MLOGD("Shell", "Matrix OS Total Heap Size: %d (%.2fkb)", totalHeapSize, totalHeapSize / 1024.0f);
...
void Shell::InitializeFolderSystem() {
  ...
  for (const auto& [appId, appInfo] : applications)
  {
    all_applications.emplace(appId, ApplicationEntry(appInfo));
  }
  ...
  std::unordered_set<uint32_t> appsInFolders;
  for (auto& [folderId, folder] : folders)
  {
    for (uint32_t appId : folder.app_ids)
    {
      appsInFolders.insert(appId);
    }
  }

  // Check if any apps are missing from the folders and add them to appropriate folders
  bool missingAppsFound = false;

  // First, check native apps in registration order
  auto& application_ids = GetApplicationIDs();
  for (const auto& [order_id, appId] : application_ids)
  {
    // If app is not in any folder, add it
    if (appsInFolders.find(appId) == appsInFolders.end())
    {
      auto appEntryIt = all_applications.find(appId);
      if (appEntryIt == all_applications.end())
      {
        continue;
      }

      uint8_t folderId = GetAppFolder(appId, appEntryIt->second);

      // Add to the appropriate folder
      folders[folderId].app_ids.push_back(appId);
      missingAppsFound = true;

      Application_Info* info = appEntryIt->second.native.info;
      MLOGD("Shell", "Added native app %s-%s to folder %d", info->author.c_str(), info->name.c_str(), folderId);
    }
  }
  ...
  {
    uint32_t appId = StringHash(py_app.info.author + "-" + py_app.info.name);
    ...
    if (appsInFolders.find(appId) == appsInFolders.end())
    {
      ...
      MLOGD("Shell", "Added Python app %s-%s to folder %d", py_app.info.author.c_str(), py_app.info.name.c_str(), folderId);
    }
  }

  // Save vectors if we loaded existing ones but found missing apps, or if no folders were loaded
  if (missingAppsFound || !foldersLoaded)
  {
...
bool Shell::EnableFolder(uint8_t folderIdx, Color color) {
  ...
  folder_colors[folderIdx] = color;

  // Ensure folder exists in the map
  if (folders.find(folderIdx) == folders.end())
  {
    folders[folderIdx] = {{}};
  }
...
void Shell::DisableFolder(uint8_t folderId) {
  ...
  for (uint32_t appId : folders[folderId].app_ids)
  {
    MoveAppToFolder(appId, 0);
  }

  // Disable folder by setting color to black
  folder_colors[folderId] = Color(0x000000);
  folders[folderId].app_ids.clear();
...
void Shell::MoveAppToFolder(uint32_t appId, uint8_t folderId) {
  ...
  {
    uint8_t oldFolderId = 255; // Track which folder we removed from

    // Remove app from current folder
    for (auto& [fid, folder] : folders)
    {
      auto it = std::find(folder.app_ids.begin(), folder.app_ids.end(), appId);
    ...
    if (oldFolderId != 255)
    {
      SaveFolderVector(oldFolderId);
    }
    SaveFolderVector(folderId);
  }
}
...
void Shell::SaveFolderVector(uint8_t folderId) {
  ...
  std::string nvs_key_str = "203 Systems-Shell-Folder-" + std::to_string(folderId) + "-Apps";
  uint32_t nvs_key = StringHash(nvs_key_str);
  std::vector<uint32_t>& app_vector = folders[folderId].app_ids;
...
void Shell::LoadFolderVector(uint8_t folderId) {
  ...
  if (stored_size_int > 0 && stored_size_int % sizeof(uint32_t) == 0)
  {
    ...
    {
      folders[folderId].app_ids = stored_apps;
    }
  }
}
...
void Shell::CleanupInvalidApps() {
  ...
  for (uint8_t folderId : all_folders)
  {
    std::vector<uint32_t>& appIds = folders[folderId].app_ids;
    std::vector<uint32_t> valid_apps;

    for (uint32_t appId : appIds)
    {
      // Check if this app exists in the all_applications map
      auto app_it = all_applications.find(appId);
      if (app_it != all_applications.end())
      {
        // App exists, keep it
        valid_apps.push_back(appId);
      }
      else
      {
        // App doesn't exist anymore, log and remove
        MLOGD("Shell", "Removing invalid app ID %X from folder %d", appId, folderId);
        any_changes = true;
      }
    }

    // Update the folder with only valid apps
    if (appIds.size() != valid_apps.size())
    {
      appIds = valid_apps;
      SaveFolderVector(folderId);
    }
...
uint8_t Shell::GetAppFolder(uint32_t appId, const ApplicationEntry& app_entry) {
  // Check if app is invisible first
  Application_Info* info = (app_entry.type == ApplicationType::Native) ? app_entry.native.info : &(app_entry.python.info->info);
  if (!info->visibility)
  {
...
void Shell::DiscoverPythonApps() {
  ...
  for (auto& py_app : python_app_infos)
  {
    ...
    if (all_applications.find(appId) != all_applications.end())
    {
      MLOGW("Shell", "Python app %s-%s conflicts with existing app, skipping", py_app.info.author.c_str(), py_app.info.name.c_str());
      continue;
    }

    // Create ApplicationEntry pointing to the stored PythonAppInfo
    all_applications.emplace(appId, ApplicationEntry(&py_app));

    MLOGD("Shell", "Registered Python app: %s-%s (ID: %X)", py_app.info.author.c_str(), py_app.info.name.c_str(), appId);
  }
