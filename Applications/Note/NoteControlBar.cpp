...
NoteControlBar::NoteControlBar(Note* notePtr, NotePad* notepad1, NotePad* notepad2, UnderglowLight* underglow1,
                               UnderglowLight* underglow2) {
  this->note = notePtr;
  ...
  else if (xy == Point(0, CTL_BAR_Y - 1))
  {
    ...
    else if (keypadInfo->state == KeypadState::Aftertouch)
    {
      if (pitch_down != 0 && pitch_down > pitch_up)
      {
        ...
        MLOGD("Note", "Pitch Bend: %d", pitchVal);
...
bool NoteControlBar::ChordControlKeyEvent(Point xy, KeypadInfo* keypadInfo) {
  ...
  if (xy.y == CTL_BAR_Y - 3 && xy.x <= 3)
  {
    if (keypadInfo->state == KeypadState::Pressed)
    {
      ...
      {
        combo.ext6 = chordExtKeyOn[0];
        combo.extMin7 = chordExtKeyOn[1];
        combo.extMaj7 = chordExtKeyOn[2];
        combo.ext9 = chordExtKeyOn[3];
      }
  ...
  if (xy.y == CTL_BAR_Y - 2 && xy.x <= 3)
  {
    if (keypadInfo->state == KeypadState::Pressed)
    {
      ...
      else
      {
        ...
        {
        case 0:
          combo.ext6 = !combo.ext6;
          break;
        case 1:
          combo.extMin7 = !combo.extMin7;
          break;
        case 2:
          combo.extMaj7 = !combo.extMaj7;
          break;
        case 3:
          combo.ext9 = !combo.ext9;
          break;
        }
      }
...
bool NoteControlBar::KeyControlKeyEvent(Point xy, KeypadInfo* keypadInfo) {
  ...
  {
    uint16_t cAlignedScaleMap = ((notePad[0]->rt->config->scale << notePad[0]->rt->config->rootKey) +
                                    ((notePad[0]->rt->config->scale & 0xFFF) >> (12 - notePad[0]->rt->config->rootKey % 12))) &
                                   0xFFF;
...
void NoteControlBar::RenderKeyControl(Point origin) {
  uint16_t cAlignedScaleMap = ((notePad[0]->rt->config->scale << notePad[0]->rt->config->rootKey) +
                                  ((notePad[0]->rt->config->scale & 0xFFF) >> (12 - notePad[0]->rt->config->rootKey % 12))) &
                                 0xFFF; // Root key should always < 12,
