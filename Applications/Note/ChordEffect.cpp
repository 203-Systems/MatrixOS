  ...
  for (uint8_t chordNote : newChordNotes)
  {
    ...
    if (noteOwner.find(chordNote) != noteOwner.end())
    {
      ...
      if (noteMap.find(previousOwner) != noteMap.end())
      {
        ...
        prevData.chordNotes.erase(std::remove(prevData.chordNotes.begin(), prevData.chordNotes.end(), chordNote),
                                  prevData.chordNotes.end());
      }
    }
...
void ChordEffect::ProcessNoteOff(const MidiPacket& packet, deque<MidiPacket>& output) {
  ...
  if (noteMap.find(root) == noteMap.end())
    return;
...
void ChordEffect::ClearChord() {
  chordCombo = {.dim = false, .min = false, .maj = false, .sus = false, .ext6 = false, .extMin7 = false, .extMaj7 = false, .ext9 = false};
  chordChanged = true;
}
