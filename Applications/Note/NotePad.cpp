...
const Color polyNoteColor[12] = {Color(0x00FFD9), Color(0xFF0097), Color(0xFFFB00), Color(0x5D00FF), Color(0xFF4B00), Color(0x009BFF),
                                 Color(0xFF003E), Color(0xAEFF00), Color(0xED00FF), Color(0xFFAE00), Color(0x1000FF), Color(0xFF1D00)};

const Color rainbowNoteColor[12] = {Color(0xFF0000), // Red
                                    Color(0xFF4000),
...
void NotePad::RemoveActiveKey(Point position) {
  activeKeys.erase(
      std::remove_if(activeKeys.begin(), activeKeys.end(), [position](const ActiveKey& key) { return key.position == position; }),
      activeKeys.end());
}
...
void NotePad::GenerateOctaveKeymap() {
  ...
  for (int8_t y = 0; y < dimension.y; y++)
  {
    ...
    for (int8_t x = 0; x < dimension.x; x++)
    {
      ...
      { // If enforce scale is true, find the next note that is in scale
        while (true)
...
bool NotePad::RenderRootNScale(Point origin) {
  ...
  for (int8_t y = 0; y < dimension.y; y++)
  {
    for (int8_t x = 0; x < dimension.x; x++)
    {
      ...
      else if (IsNoteActive(note) || rt->midiPipeline.IsNoteActive(note) || IsNoteHighlighted(note))
      { // If find the note is currently active. Show it as white
