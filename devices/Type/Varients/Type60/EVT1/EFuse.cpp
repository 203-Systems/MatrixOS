...
void BurnEFuse() {
  ...
  MatrixOS::Logging::LogDebug("BurnEFuse", "Burning Device Info - Status: %s", esp_err_to_name(status));
  ...
  MatrixOS::Logging::LogDebug("BurnEFuse", "Buriing EFUSE_WR_DIS_SPI_BOOT_CRYPT_CNT - Status: %s",
                              esp_err_to_name(status));

  // Write protection for USB_EXCHG_PINS,
  status = esp_efuse_write_field_bit(ESP_EFUSE_WR_DIS_USB_EXCHG_PINS);
  MatrixOS::Logging::LogDebug("BurnEFuse", "Buriing ESP_EFUSE_WR_DIS_USB_EXCHG_PINS - Status: %s",
                              esp_err_to_name(status));

  // Write protection for Device Info
  status = esp_efuse_set_write_protect(EFUSE_BLK3);
  MatrixOS::Logging::LogDebug("BurnEFuse", "Block 3 (Device Info) write protected - Status: %s",
                              esp_err_to_name(status));
