...
uint8_t target_mac[ESP_NOW_ETH_ALEN] = { 0x7C, 0xDF, 0xA1, 0x3E, 0xD2, 0xA2}; //Current this is hard coded to my own reciver. A pairing system is required for future
...
namespace Device
{
    namespace ESPNOW
    {
        ...
        void Init()
        {
            ...
            {
                ESP_LOGI("espnow_init", "Target Mac wrote: %#02X:%#02X:%#02X:%#02X:%#02X:%#02X", target_mac[0], target_mac[1], target_mac[2], target_mac[3], target_mac[4], target_mac[5]);
                nvs_set_blob(esp_now_nvs, "target_mac", &target_mac, mac_size);
                nvs_commit(esp_now_nvs);
            }
            else
            {
                ESP_LOGI("espnow_init", "Target Mac Load: %#02X:%#02X:%#02X:%#02X:%#02X:%#02X", target_mac[0], target_mac[1], target_mac[2], target_mac[3], target_mac[4], target_mac[5]);
            }
        ...
        void ESPNOW_Recv_CB(const uint8_t *mac_addr, const uint8_t *data, int len)
        {
            // ESP_LOGI("ESPNOW", "Message recivied from %#02X:%#02X:%#02X:%#02X:%#02X:%#02X", mac_addr[0], mac_addr[1], mac_addr[2], mac_addr[3], mac_addr[4], mac_addr[5]);
