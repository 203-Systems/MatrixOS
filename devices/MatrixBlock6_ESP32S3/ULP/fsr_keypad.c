...
volatile uint16_t threshold[X_SIZE][Y_SIZE];

#define DEFAULT_THRESHOLD 2048
// #define ROLLING_THRESHOLD_LIFE_TIME 4096
#define IIF_LENGTH 8
...
int main(void)
{
  ...
  for (uint8_t x = 0; x < X_SIZE; x++)
  {
    ...
    for (uint8_t y = 0; y < Y_SIZE; y++)
    {
      threshold[x][y] = DEFAULT_THRESHOLD;
      result[x][y] = 0;
    }
  }
  while(true)
  {
    for (uint8_t x = 0; x < X_SIZE; x++)
    {
      ...
      for (uint8_t y = 0; y < Y_SIZE; y++)
      {
        ...
        threshold[x][y] = (reading_1 + reading_2) / 2;
        // if (result[x][y] < (threshold[x][y] + 512))
        // {
        //   threshold[x][y] = (threshold[x][y] * (ROLLING_THRESHOLD_LIFE_TIME - 1) + result[x][y]) / ROLLING_THRESHOLD_LIFE_TIME;
        // }
      }
      ulp_riscv_gpio_output_level(keypad_write_pins[x], 0);
    }
