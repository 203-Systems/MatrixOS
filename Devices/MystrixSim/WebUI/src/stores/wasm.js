// WASM module lifecycle store for MystrixSim
...
const looksLikeMatrixOSLog = (args) => {
  ...
  return text.startsWith('[MystrixSim]') || /^[DIWEV]\s*\(\d+\)/.test(text)
}
...
function terminateWasmRuntime() {
  ...
  try {
    ...
    console.warn('[MystrixSim] Error terminating pthreads:', e)
  }
...
export async function restartWasm() {
  ...
  console.info('[MystrixSim] Runtime hot-restart initiated')
