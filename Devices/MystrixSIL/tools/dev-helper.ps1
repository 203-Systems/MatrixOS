...
function Enable-Emscripten {
  if ((Test-CommandAvailable 'emcmake') -and (Test-CommandAvailable 'emcc')) {
    Write-Host '[MystrixSIL] Emscripten toolchain already available in PATH.'
    return
  }
  ...
  foreach ($root in $roots) {
    if (Test-Path (Join-Path $root 'emsdk.py')) {
      Write-Host "[MystrixSIL] Bootstrapping Emscripten from $root"
  ...
  throw @"
MystrixSIL requires the Emscripten toolchain, but emcmake/emcc were not found.

Install or activate Emscripten first, for example:
  https://emscripten.org/docs/getting_started/downloads.html

Then re-run:
  make DEVICE=MystrixSIL setup
"@
}
...
function Invoke-Step {
  ...
  Write-Host "[MystrixSIL] $FilePath $($ArgumentList -join ' ')"
...
try {
  switch ($Action) {
    ...
    'build' {
      ...
      $jsSrc = Join-Path $BuildDir 'Devices\MystrixSIL\MatrixOSHost.js'
      $wasmSrc = Join-Path $BuildDir 'Devices\MystrixSIL\MatrixOSHost.wasm'
      ...
      Write-Host "[MystrixSIL] Copied MatrixOSHost.js and MatrixOSHost.wasm to $publicDir"
    }
