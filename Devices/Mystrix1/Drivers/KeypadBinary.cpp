...
void Init() {
  ...
  {
    io_conf.pin_bit_mask |= (1ULL << keypadReadPins[y]);
  }
  gpio_config(&io_conf);
  ...
  for (uint8_t x = 0; x < X_SIZE; x++)
  {
    gpio_set_level(keypadWritePins[x], 0);
  }
}
...
IRAM_ATTR bool Scan() {
  for (uint8_t x = 0; x < X_SIZE; x++)
  {
    gpio_set_level(keypadWritePins[x], 1);
    for (uint8_t y = 0; y < Y_SIZE; y++)
    {
      Fract16 reading = gpio_get_level(keypadReadPins[y]) * FRACT16_MAX;
      // MLOGD("Keypad", "%d %d Read: %d", x, y, gpio_get_level(keypadReadPins[y]));
      bool updated = keypadState[x][y].Update(binaryConfig, reading);
      if (updated)
      {
