...
void ForceCalibration::Setup(const vector<string>& args) {
  Device::Rotate(Direction::UP, true);
...
void ForceCalibration::SetOffset(CalibrationType type) {
  ...
  int32_t displayValue = abs(offset);

  UIButton minusBtn;
  minusBtn.SetName("Subtract one");
  minusBtn.SetColorFunc([&]() -> Color { return Color(0xFF00FF).DimIfNot(offset > lowerBound); });
  ...
  minusBtn.OnPress([&]() -> void {
    ...
    displayValue = abs(offset);
  });
  offsetUI.AddUIComponent(minusBtn, Point(0, 7));

  UIButton plusBtn;
  plusBtn.SetName("Add one");
  plusBtn.SetColorFunc([&]() -> Color { return Color(0x00FFFF).DimIfNot(offset < upperBound); });
  ...
  offsetDisplay.SetColorFunc(
      [&](uint16_t digit) -> Color { return digit % 2 ? Color::White : (offset < 0 ? Color(0xFF00FF) : Color(0x00FFFF)); });
  offsetDisplay.SetDigits(2);
  offsetDisplay.SetValuePointer((int32_t*)&displayValue);
