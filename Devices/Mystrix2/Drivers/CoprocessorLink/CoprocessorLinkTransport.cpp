...
UartTransport::UartTransport(uart_port_t uartNum, int txPin, int rxPin, uint32_t baudRate, int rxBufferSize, int txBufferSize)
    : uartNum_(uartNum), txPin_(txPin), rxPin_(rxPin), baudRate_(baudRate), rxBufferSize_(rxBufferSize), txBufferSize_(txBufferSize) {}
...
bool UartTransport::begin() {
  ...
  ESP_LOGI(kTag, "uart begin: port=%d tx=%d rx=%d baud=%" PRIu32 " rx_buf=%d tx_buf=%d", (int)uartNum_, txPin_, rxPin_, baudRate_,
           rxBufferSize_, txBufferSize_);
...
bool UartTransport::write(const uint8_t* data, size_t len) {
  ...
  ESP_LOGD(kTag, "uart tx len=%u first=%02X %02X %02X %02X", (unsigned)len, len > 0 ? data[0] : 0, len > 1 ? data[1] : 0,
...
bool UartTransport::read(uint8_t* data, size_t len, uint32_t timeoutMs) {
  ...
  while (received < len)
  {
    ...
    const int got = uart_read_bytes(uartNum_, data + received, len - received, pdMS_TO_TICKS(remainMs > 10U ? 10U : remainMs));
    if (got > 0)
    {
      received += (size_t)got;
    }
  ...
  ESP_LOGD(kTag, "uart rx len=%u first=%02X %02X %02X %02X", (unsigned)len, len > 0 ? data[0] : 0, len > 1 ? data[1] : 0,
           len > 2 ? data[2] : 0, len > 3 ? data[3] : 0);
  return true;
}
...
size_t UartTransport::readAvailable(uint8_t* data, size_t maxLen) {
  ...
  if (got > 0)
  {
    ESP_LOGW(kTag, "uart stray rx len=%u first=%02X %02X %02X %02X", (unsigned)got, got > 0 ? data[0] : 0, got > 1 ? data[1] : 0,
