...
inline uint32_t FNV1aHash(const char* ptr, size_t length) // Implementation of FNV-1a Hash
{
...
template <uint32_t N, uint32_t I> struct HashHelper {
  constexpr static uint32_t Calculate(const char (&str)[N]) {
    return (HashHelper<N, I - 1>::Calculate(str) ^ (str[I - 1] & 0xFF)) * FNV_PRIME;
  }
};

template <uint32_t N> struct HashHelper<N, 1> {
  constexpr static uint32_t Calculate(const char (&str)[N]) {
    return (FNV_OFFSET_BASIS ^ (str[0] & 0xFF)) * FNV_PRIME;
  }
};

template <uint32_t N> constexpr static uint32_t StaticHash(const char (&str)[N]) {
