...
class UINumberModifier: public UIComponent
{
    ...
    UINumberModifier(Color color, uint8_t length, int32_t* value, int32_t* modifiers, uint8_t* control_gradient, int32_t lower_limit = INT_MIN, int32_t upper_limit = INT_MAX)
    {
        this->color = color;
        this->length = length;
        this->value = value;
