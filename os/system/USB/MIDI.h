...
struct alignas(4) midiBufferData {
    uint8_t data1;
    uint8_t data2;
    uint8_t data3;
    uint8_t data4;
...
namespace MatrixOS::USB
{
  class MIDI : public UsbInterface
  {
    ...
    inline static map<uint8_t, midiBufferData> out_ep_buffer = {};

    MIDI();
    vector<MidiPort>* Init(string interface_name="", uint16_t ep_size = 64, uint8_t cable_nums = 1); // Use "" as interface name will use default string generated by the OS
    vector<MidiPort>* InitMultiCable(uint8_t cable_nums, string interface_name = "", uint16_t ep_size = 64); // Create multiple cables in the interface
    ~MIDI();
