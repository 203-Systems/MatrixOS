...
- `Released`: `velocity` 可以写入 release velocity；如果当前硬件或算法没有 release velocity，则使用固定 release velocity 或最后一次 strike velocity 作为兼容 fallback。

应用层不应该再用 `pressure` 当 note-on velocity。正确消费方式是：Note On 读 `velocity`，Aftertouch 读 `pressure`，Note Off 如需 release velocity 读 `velocity`。
...
flowchart LR
  ...
  fast --> vel[Strike/release velocity tracker]
...
- `pressure`: 当前绝对压力。`Pressed` 时是按下瞬间压力，`Aftertouch` 时持续变化，`Released` 时可以清零或保留最后值，但应用不应把它当 release velocity。
- `velocity`: 事件速度。`Pressed` 是 strike velocity，`Released` 是 release velocity，其他 active 状态保持 strike velocity。
...
3. 如果启用 release velocity，收集释放前后 2 到 4 个 samples，计算压力下降速度。
4. 设备层生成 `releaseEdge = true`，并把 `velocity` 写成 release velocity。没有实现 release velocity 时，可先写固定值 64 或最后 strike velocity。
...
1. ULP 增加每格 history ring buffer，例如 8 到 16 帧：
  - `rawHistory[historySize][X_SIZE][Y_SIZE]`: 最近 ADC 缩放值，用于 strike/release velocity。
...
if (keypadInfo->state == KeypadState::Pressed) {
  ...
  releaseVelocity = keypadInfo->velocity.to7bits();
  SendNoteOff(note, releaseVelocity);
}
```
...
- Release velocity mode 或 fixed release velocity。
...
3. 让 FSR velocity tracker 从 ULP history 读样本，完成 strike velocity / release velocity 计算。
4. 定义 `UserKeypadConfig`、默认值、enum code、profile / LUT 表，但第一版只 hardcode 默认配置。
5. `Pressed` 写 `velocity`，`Aftertouch` 更新 `pressure`，`Released` 写 release velocity 或兼容 fallback。
...
3. 统一把 `Released` 的 Note Off 改为读取 `keypadInfo->velocity` 作为 release velocity。
...
- MIDI monitor 里事件顺序正确：NoteOn velocity，然后 pressure aftertouch，release 时 NoteOff release velocity。
...
- 验证 release velocity 在 `Released` 事件写入 `velocity`。
- 验证 aftertouch threshold 只按 `pressure` 变化触发。

### 设备级

- Force visualizer 增加显示 strike velocity 和 pressure 的模式。
- 日志输出：key id、ULP count delta、history index、raw force history、filtered pressure、strike velocity、release velocity、state。
...
- MIDI monitor: 验证事件顺序为 NoteOn velocity，然后 pressure aftertouch 更新，release 时 NoteOff release velocity。
...
- release velocity 复用 `velocity` 字段时，应用必须按 `state` 解释字段含义。
